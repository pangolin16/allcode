...
function generateTables() {
    ...
    const filteredDates = originalArray.filter(
        value => typeof value === "string" && /^\d{2}\.\d{2}\.\d{2}$/.test(value)
    );
    ...
    // tab1: Each row for each date; material column is filled (if available), date column is filled, comment column is editable
    let html2 = "<table id='tab1'><tbody>";
    // We'll use filteredDates.length rows, and for each row, we show material (if available), date, comment
    for (let i = 0; i < filteredDates.length; i++) {
        let dateVal = filteredDates[i];
        html2 += "<tr>";
        // Try to find material associated with the date (previous value in originalArray), otherwise leave blank
        let materialVal = "";
        let idxInOriginal = originalArray.indexOf(dateVal);
        if (idxInOriginal > 0) {
            let prevVal = originalArray[idxInOriginal - 1];
            if (dataMap[prevVal]) {
                materialVal = prevVal;
            }
        }
        html2 += `<td>${materialVal ? dataMap[materialVal] : ""}</td>`;
        html2 += `<td>${dateVal}</td>`;
        // Editable comment
        const commentKey = dateVal + "_" + i;
        const commentVal = savedComments[commentKey] || "";
        html2 += `<td contenteditable="true" class="comment" data-key="${commentKey}">${commentVal}</td>`;
        html2 += "</tr>";
    }
    html2 += "</tbody></table>";
...
// Highlight duplicates in the column left of the dates column (assume column 0 if date is column 1)
function colour() {
    ...
    // Count occurrences of each value in column left of dates
    for (let i = 0; i < rows.length; i++) {
        const cells = rows[i].getElementsByTagName('td');
        if (cells.length > 1) { // need at least 2 columns
            const value = cells[0].textContent.trim(); // Column left of dates is column 0
            valueCount[value] = (valueCount[value] || 0) + 1;
        }
    }

    // Highlight duplicates in column left of dates (column 0)
    for (let i = 0; i < rows.length; i++) {
        const cells = rows[i].getElementsByTagName('td');
        if (cells.length > 1) {
            ...
            if (valueCount[value] > 1) {
                ...
                cells[0].style.backgroundColor = colors[index % colors.length];
            } else {
