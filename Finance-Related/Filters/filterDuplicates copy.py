...
# Parse each line: convert to int if possible, else keep as string
def parse_line(line):
    line = line.strip()
    if not line:
        return None
...
original_list = [parse_line(line) for line in str_data.strip().splitlines() if line.strip()]
# Remove None values (from blank lines)
original_list = [x for x in original_list if x is not None]

# print(original_list)
# Output: [1319, 1519, 1786, 2345, 'A&D Holon Holdings', 'Adobe', 'adobe', 'Adobe Inc.']
str_data_formatted = "\n".join(str(item) for item in original_list)
print(str_data_formatted)