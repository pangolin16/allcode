...
with tab1:
    ...
    with col1:
        ticker1 = st.text_input("Stock 1", "AAPL")
        if ticker1:
            tickers.append(ticker1.upper())

    with col2:
        ticker2 = st.text_input("Stock 2", "MSFT")
        if ticker2:
            tickers.append(ticker2.upper())

    with col3:
        ticker3 = st.text_input("Stock 3", "")
        if ticker3:
            tickers.append(ticker3.upper())

    with col4:
        ticker4 = st.text_input("Stock 4", "")
        if ticker4:
...
with tab2:
    ...
    with col1:
        m_ticker1 = st.text_input("Company 1", "AAPL", key="m1")
        if m_ticker1:
            metric_tickers.append(m_ticker1.upper())

    with col2:
        m_ticker2 = st.text_input("Company 2", "MSFT", key="m2")
        if m_ticker2:
            metric_tickers.append(m_ticker2.upper())

    with col3:
        m_ticker3 = st.text_input("Company 3", "", key="m3")
        if m_ticker3:
            metric_tickers.append(m_ticker3.upper())

    with col4:
        m_ticker4 = st.text_input("Company 4", "", key="m4")
        if m_ticker4:
            ...
            with st.spinner("Fetching financial data..."):
                ...
                for ticker in metric_tickers:
                    ...
                    if income_statement is not None and not income_statement.empty:
                        # Sort by date ascending (oldest first) and reset index
                    ...
                    for ticker in metric_tickers:
                        if ticker in metrics_data:
                            ...
                            if revenue_col is not None:
                                ...
                                quarter_labels = format_to_quarter(df.index)
